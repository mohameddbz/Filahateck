...
const { fetchMsaviImage } = require('./../../services/Santinel/MsaviService'); // Ensure you have a MSAVI service
const requestService = require('./../../services/Precision/requestService'); 
const { sequelize } = require('../../config/config'); 

async function createMsaviRequest(req, res) {
  ...
  try {
    ...
    if (subscription.length === 0) {
      return res.status(403).json({ message: "Aucun abonnement trouvé pour cet utilisateur et pour MSAVI." });
    }

    const nombreDeRequetes = 15; // or subscription[0].nombre_de_requete;
    if (nombreDeRequetes <= 0) {
      return res.status(403).json({ message: "Votre abonnement ne permet aucune requête pour MSAVI." });
    }
    ...
    if (lastRequest.length > 0) {
      ...
      if (new Date() < nextAvailableDate) {
        return res.status(403).json({
          message: `Vous devez attendre ${daysBetweenRequests} jours avant une nouvelle requête pour MSAVI.`,
    ...
    const imageData = await fetchMsaviImage(
      JSON.parse(bbox),
      timeRange,
    ...
    if (!imageData) {
      return res.status(404).json({ message: 'No MSAVI image data returned.' });
    }
    ...
    const imageName = `msavi-image-${uuidv4()}.jpg`;
    ...
    return res.status(201).json({
      message: 'Request created successfully for MSAVI.',
      data: createdRequest,
    });
  } catch (error) {
    console.error('Error in createMsaviRequest:', error);
    return res.status(500).json({ message: error.message });
  }
}

module.exports = { createMsaviRequest };
