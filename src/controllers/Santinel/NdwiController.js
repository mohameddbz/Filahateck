...
const { sequelize } = require('../../config/config'); // Adjust based on your setup
...
async function createNdwiRequest(req, res) {
  ...
  try {
    ...
    // Check the last NDWI request date
    ...
    if (!imageData) {
      return res.status(404).json({ message: 'No NDWI image data returned.' });
    }
